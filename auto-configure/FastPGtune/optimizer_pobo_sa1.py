...
class EHVIBO:
    ...
    # def recommend(self, fixed_features, q, rr_cons):
    #     # 假设输出为2维：[fitness, recall]
    #
    #     with torch.no_grad():
    #         pred = self.model.posterior(self.X_init).mean
    #         # 正确取第二个目标维度（recall）的最大值作为best_f
    #         best_f = pred[..., 1].max().item()  # 修改索引为1
    #
    #     # 定义约束函数：fitness（第0维）需大于rr_cons
    #     def constraint_fn(Y: torch.Tensor) -> torch.Tensor:
    #         return Y[..., 0] - rr_cons  # 第0维是fitness
    #
    #     # 创建约束Expected Improvement采集函数
    #     acq_func = ConstrainedExpectedImprovement(
    #         model=self.model,
    #         objective_index=1,  # 优化目标为recall（第1维）
    #         best_f=best_f,
    #         constraints={0: (None, constraint_fn)},  # 应用约束到第0维
    #     )
    #
    #     # 优化采集函数获取候选点
    #     candidate, ei = optimize_acqf(
...
class PollingBayesianOptimization:
    ...
    def init_sample(self):
        for k in self.remain_types:
            ...
            if y is not None:
                ...
                print(f"Warning: y is None for key {k}")
            # self.X[k] = self.X[k] + x
            # self.Y[k] = self.Y[k] + y.tolist()
            # print(self.X)
    ...
    def step(self,):
        ...
        sp.run(f'echo {list(self.chosen_ref_k.values())} {self.remain_types}>> pobo_record.log', shell=True, stdout=sp.PIPE)
        time_0 = time.time()
        ...
        print(new_y)
        time_2 = time.time()
        log_dir = "/home/yinzh/VDTuner/NSG/msong/q_10"
        os.makedirs(log_dir, exist_ok=True)
        log_file_path = os.path.join(log_dir, "NSG_10_msong.log")
        with open(log_file_path, 'a') as f:
            f.write("这一轮推荐参数耗时: {:.4f} seconds\n".format(float(time_1 - time_0)))
            f.write("这一轮获取响应耗时: {:.4f} seconds\n".format(float(time_2 - time_1)))  
    ...
    def step1(self, n):
        ...
        # 记录当前各类型参考值和剩余类型到日志
        sp.run(
            f"echo {list(self.chosen_ref_k.values())} {self.remain_types} >> pobo_record1.log",
            shell=True,
            stdout=sp.PIPE
        )
        ...
        # 轮询获取 n 个候选
        time_0 = time.time()
        polling_k, new_x = self.rr_polling1(n)
        time_1 = time.time()
        print(new_x)
        # 从环境中获取对应响应
        new_y = self.env.get_state1(new_x,n)
        time_2 = time.time()
        log_dir = "/home/yinzh/VDTuner/NSG/msong/q_10"
        os.makedirs(log_dir, exist_ok=True)
        log_file_path = os.path.join(log_dir, "NSG_10_msong.log")
        ...
        print(new_y)
        # 更新历史样本
        self.X[polling_k] += new_x.tolist()
        self.Y[polling_k] += new_y.tolist()
    ...
    def rr_polling(self,):
        ...
        new_x, ei, new_mean, new_std = self.vbo.recommend(fixed_features, 2, self.threshold)
    ...
    def rr_polling1(self, n):
        # 1. 计算当前轮询的类型索引和类型值
        polling_idx = self.polling_round_num % len(self.remain_types)
        polling_k = self.remain_types[polling_idx]

        # 2. 构造“固定特征”：将除当前类型相关 knobs 之外的维度都保持默认值
        fixed_idxs = [
            i for i in range(self.knob_num)
            if i not in (self.polling_sys + self.polling_index[polling_k])
        ]
        default_vals = np.array(self.default_conf)[fixed_idxs]
        fixed_features = dict(zip(fixed_idxs, default_vals))

        # 3. 将第 0 号 knob（index_type）设置为当前轮询类型的编码值
        fixed_features[0] = self.env.knob_stand.scale_forward('index_type', polling_k)

        # 4. 调用 VBO 推荐接口，批量大小由参数 n 决定
        new_x, ei, new_mean, new_std = self.vbo.recommend(
            fixed_features,
            q=n,                # 批量大小改为 n
            rr_cons=self.threshold
        )
