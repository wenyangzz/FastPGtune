...
class EHVIBO:
    ...
    def recommend(self, fixed_features, q, rr_cons):
        ...
        with open(log_file_path, 'a') as f:
            f.write("时间4: {:.4f} seconds\n".format(float(time_4 - time_3)))
            f.write("时间5: {:.4f} seconds\n".format(float(time_5 - time_4)))
...
class PollingBayesianOptimization:
    ...
    # 以 'w' 模式打开文件一次，初始化文件内容
        ...
        with open(log_file_path, 'a') as f:
            ...
            f.write("time_2系统时间是{}\n".format(time_2))
            f.write("time_3系统时间是{}\n".format(time_3))
            f.write("推荐参数时间: {:.4f} seconds\n".format(float(time_1 - time_0)))
            f.write("获取响应时间: {:.4f} seconds\n".format(float(time_2 - time_1)))
            f.write("模型更新时间: {:.4f} seconds\n".format(float(time_3 - time_2)))
    def step1(self, n):
        """
        执行一次轮询采样，并对模型进行更新。
        `n` 控制每次推荐的批量大小。
    ...
    def rr_polling(self,):
        ...
        # time_2 = time.time()
        with open(log_file_path, 'a') as f:
            f.write("时间1: {:.4f} seconds\n".format(float(time_1 - time_0)))
            f.write("recommend: {:.4f} seconds\n".format(float(time_2 - time_1)))
    ...
    def rr_polling1(self, n):
        ...
        fixed_features[0] = self.env.knob_stand.scale_forward('index_type', polling_k)
        time_1 = time.time()
        new_x, ei, new_mean, new_std = self.vbo.recommend(fixed_features, n, self.threshold)
        time_2 = time.time()
        self.polling_round_num += 1
        # print(self.polling_round_num)
