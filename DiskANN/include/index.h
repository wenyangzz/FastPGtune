...
template <typename T, typename TagT = uint32_t, typename LabelT = uint32_t> class Index : public AbstractIndex
{
  ...
  public:
    //添加的函数
    // DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> iterate_to_fixed_point2(InMemQueryScratch<T> *scratch, const uint32_t Lindex,
    //                                                      const std::vector<uint32_t> &init_ids, bool use_filter,
    //                                                      const std::vector<LabelT> &filters,DistList *d1,bool search_invocation);

    // DISKANN_DLLEXPORT void search_for_point_and_prune2(int location, uint32_t Lindex, std::vector<uint32_t> &pruned_list,
    //                                 InMemQueryScratch<T> *scratch, DistList *d1,bool use_filter = false,
    //                                 uint32_t filteredLindex = 0);

    DISKANN_DLLEXPORT void end();

    DISKANN_DLLEXPORT void initializeProcessing(std::vector<uint32_t>& visit_order); 
    //原来
    DISKANN_DLLEXPORT void add_point(const std::vector<uint32_t>& visit_order,int64_t node_ctr);
    //现在
    DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> add_point1(const std::vector<uint32_t>& visit_order,DistList *dl,int64_t node_ctr);

    DISKANN_DLLEXPORT unsigned long long globalOptimizationPrune(const std::vector<uint32_t>& visit_order);
  ...
  protected:
    ...
    std::pair<uint32_t, uint32_t> iterate_to_fixed_point2(InMemQueryScratch<T> *scratch, const uint32_t Lindex,
                                                         const std::vector<uint32_t> &init_ids, bool use_filter,
                                                         const std::vector<LabelT> &filters,DistList *d1,bool search_invocation);

    std::pair<uint32_t, uint32_t> search_for_point_and_prune2(int location, uint32_t Lindex, std::vector<uint32_t> &pruned_list,
                                    InMemQueryScratch<T> *scratch, DistList *d1,bool use_filter = false,
                                    uint32_t filteredLindex = 0);

    // void end();

    // void initializeProcessing(std::vector<uint32_t>& visit_order); 

    // void add_point(const std::vector<uint32_t>& visit_order,DistList *dl,int64_t node_ctr);

    // void globalOptimizationPrune(const std::vector<uint32_t>& visit_order);

    uint32_t prune_neighbors(const uint32_t location, std::vector<Neighbor> &pool, std::vector<uint32_t> &pruned_list,
                         InMemQueryScratch<T> *scratch);

    uint32_t prune_neighbors(const uint32_t location, std::vector<Neighbor> &pool, const uint32_t range,
    ...
    uint32_t occlude_list(const uint32_t location, std::vector<Neighbor> &pool, const float alpha, const uint32_t degree,
    ...
    uint32_t inter_insert(uint32_t n, std::vector<uint32_t> &pruned_list, const uint32_t range,
                      InMemQueryScratch<T> *scratch);

    uint32_t inter_insert(uint32_t n, std::vector<uint32_t> &pruned_list, InMemQueryScratch<T> *scratch);
  ...
  private:
    ...
    //添加DisitedListPool *disited_list_pool_;
