...
class AbstractIndex
{
  public:
    ...
    virtual void initializeProcessing(std::vector<uint32_t>& visit_order)=0; 
    //原来
    virtual void add_point(const std::vector<uint32_t>& visit_order,int64_t node_ctr)=0;
    //现在 
    virtual std::pair<uint32_t, uint32_t> add_point1(const std::vector<uint32_t>& visit_order,DistList *dl,int64_t node_ctr)=0;


    virtual unsigned long long globalOptimizationPrune(const std::vector<uint32_t>& visit_order)=0;
