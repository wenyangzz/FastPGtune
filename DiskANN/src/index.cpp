                                                      ...
                                                      defaults::GRAPH_SLACK_FACTOR * 1.05)))
{
    ...
    // std::cout << "num_frozen_pts  " << num_frozen_pts << "\n";
    // for(int i=0;i<_max_points;i++)
    // {
    //     auto nbrs = _graph_store->get_neighbours(i);
    //     for (auto id : nbrs)
    //         {
    //              diskann::cout << i << " "<< id<< std::endl;
    //         }  
    // }
}
...
//添加dist_pool
//修改iterate_to_fixed_point2
template <typename T, typename TagT, typename LabelT>
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::iterate_to_fixed_point2(
    InMemQueryScratch<T> *scratch, const uint32_t Lsize, const std::vector<uint32_t> &init_ids, bool use_filter,
    const std::vector<LabelT> &filter_labels,DistList *dl,bool search_invocation)
{
    std::vector<Neighbor> &expanded_nodes = scratch->pool();
    NeighborPriorityQueue &best_L_nodes = scratch->best_l_nodes();
    best_L_nodes.reserve(Lsize);
    ...
    std::vector<uint32_t> &id_scratch = scratch->id_scratch();
    std::vector<float> &dist_scratch = scratch->dist_scratch();
    std::vector<uint32_t> id_disted ;
    //std::vector<float> dist_disted ;
    id_disted.reserve(_max_points);
    //dist_disted.reserve(_max_points);
    assert(id_scratch.size() == 0);
    ...
    for (auto id : init_ids)
    {
        ...
        if (is_not_visited(id))
        {
            ...
            uint32_t ids[] = {id};
            if (id < _max_points){
                float temp_d = dist_array[id];
                if(temp_d < 0){
                    float distances[] = {std::numeric_limits<float>::max()};
                    _pq_data_store->get_distance(aligned_query, ids, 1, distances, scratch);
                    cmps ++;
                    distance = distances[0];
                    dist_array[id] = distances[0]; 
                    record->push_back(id);
                }else{
                    distance = temp_d;
                }
    ...
    while (best_L_nodes.has_unexpanded_node())
    {
       ...
       cmps += (uint32_t)id_scratch.size();
        ...
        {
            best_L_nodes.insert(Neighbor(id_scratch[m], dist_scratch[m]));
            if(id_scratch[m]<_max_points)
            {
               dist_array[id_scratch[m]] = dist_scratch[m]; 
               record->push_back(id_scratch[m]);
            }
        }
        for (size_t m = 0; m < id_disted.size(); ++m)
        {
            best_L_nodes.insert(Neighbor(id_disted[m], dist_array[id_disted[m]]));
        }
    }
    return std::make_pair(hops, cmps);
}

//修改search_for_point_and_prune
template <typename T, typename TagT, typename LabelT>
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::search_for_point_and_prune2(int location, uint32_t Lindex,
                                                        std::vector<uint32_t> &pruned_list,
                                                        InMemQueryScratch<T> *scratch, DistList *dl,bool use_filter,
                                                        uint32_t filteredLindex)
{
    const std::vector<uint32_t> init_ids = get_init_ids();
    const std::vector<LabelT> unused_filter_label;
    // auto result = index.iterate_to_fixed_point2(
    // scratch, Lsize, init_ids, use_filter, filter_labels, dl, search_invocation);
    // uint32_t hops = result.first;
    // uint32_t cmps = result.second;
    ...
    {
        _data_store->get_vector(location, scratch->aligned_query());
        auto cmps = iterate_to_fixed_point2(scratch, Lindex, init_ids, false, unused_filter_label,dl,false);
        result.first = cmps.first;
        result.second = cmps.second; 
    }
    else
    {
        std::shared_lock<std::shared_timed_mutex> tl(_tag_lock, std::defer_lock);
        ...
        _data_store->get_vector(location, scratch->aligned_query());
        auto cmps = iterate_to_fixed_point2(scratch, filteredLindex, filter_specific_start_nodes, true,
                               _location_to_labels[location],dl,false);
        ...
        _data_store->get_vector(location, scratch->aligned_query());
        auto cmps_1 = iterate_to_fixed_point2(scratch, Lindex, init_ids, false, unused_filter_label,dl,false);
        ...
        std::copy(best_candidate_pool.begin(), best_candidate_pool.end(), std::back_inserter(scratch->pool()));
        //记录计算量
        cmps.first = cmps_1.first + cmps.first;
        cmps.second = cmps_1.second + cmps.second;
        result.first = cmps.first;
        result.second = cmps.second;  
    }
    auto &pool = scratch->pool();
    ...
    //统计剪枝的距离计算
    result.first = prune_neighbors(location, pool, pruned_list, scratch);
...
template <typename T, typename TagT, typename LabelT>
uint32_t Index<T, TagT, LabelT>::occlude_list(const uint32_t location, std::vector<Neighbor> &pool, const float alpha,
...
template <typename T, typename TagT, typename LabelT>
uint32_t Index<T, TagT, LabelT>::prune_neighbors(const uint32_t location, std::vector<Neighbor> &pool,
                                             std::vector<uint32_t> &pruned_list, InMemQueryScratch<T> *scratch)
{
   uint32_t prune_coms = 0;
   prune_coms= prune_neighbors(location, pool, _indexingRange, _indexingMaxC, _indexingAlpha, pruned_list, scratch);
   return prune_coms;
}

template <typename T, typename TagT, typename LabelT>
uint32_t Index<T, TagT, LabelT>::prune_neighbors(const uint32_t location, std::vector<Neighbor> &pool, const uint32_t range,
...
{   
    ...
    prune_coms+=occlude_list(location, pool, alpha, range, max_candidate_size, pruned_list, scratch);
...
template <typename T, typename TagT, typename LabelT>
uint32_t Index<T, TagT, LabelT>::inter_insert(uint32_t n, std::vector<uint32_t> &pruned_list, const uint32_t range,
                                          InMemQueryScratch<T> *scratch)
{
    ...
    for (auto des : src_pool)
    {
        ...
        if (prune_needed)
        {
            ...
            prune_coms += prune_neighbors(des, dummy_pool, new_out_neighbors, scratch);
            {
                LockGuard guard(_locks[des]);
...
template <typename T, typename TagT, typename LabelT>
uint32_t Index<T, TagT, LabelT>::inter_insert(uint32_t n, std::vector<uint32_t> &pruned_list, InMemQueryScratch<T> *scratch)
{
    uint32_t prune_coms = 0;
    prune_coms=inter_insert(n, pruned_list, _indexingRange, scratch);
    return prune_coms;
}
...
//初始化进程数量以及顺序
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::initializeProcessing(std::vector<uint32_t>& visit_order) {
    // 设置 OpenMP 线程数
    uint32_t num_threads = _indexingThreads;
    if (num_threads != 0)
        omp_set_num_threads(num_threads);
    ...
    visit_order.clear();
    visit_order.reserve(_nd + _num_frozen_pts);
    for (uint32_t i = 0; i < static_cast<uint32_t>(_nd); i++) {
        visit_order.emplace_back(i);
    }
    for (uint32_t frozen = static_cast<uint32_t>(_max_points);
         frozen < _max_points + _num_frozen_pts;
         frozen++) {
        visit_order.emplace_back(frozen);
    }

    // 设置起始点，如果有冻结点则第一个冻结点作为起点，否则计算入口点
    if (_num_frozen_pts > 0)
        _start = static_cast<uint32_t>(_max_points);
    else
        _start = calculate_entry_point();
    // for(int i=0;i<_max_points;i++)
    // {
...
//原来的构建函数
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::add_point(const std::vector<uint32_t>& visit_order,int64_t node_ctr) {
    // 第一次并行遍历，构建初始图
        auto node = visit_order[node_ctr];

        // 使用临时查询空间（scratch space）
        ScratchStoreManager<InMemQueryScratch<T>> manager(_query_scratch);
        auto scratch = manager.scratch_space();
        std::vector<uint32_t> pruned_list;
        //     for(int i=0;i<_max_points;i++)
        ...
        //         }  
        // }    
        // 根据是否使用过滤索引调用对应的搜索函数，得到剔除后的邻居列表
        if (_filtered_index) {
            if(node>=_max_points)
            {
                search_for_point_and_prune(node, _indexingQueueSize, pruned_list, scratch,true, _filterIndexingQueueSize);  
            ...
            // search_for_point_and_prune2(node, _indexingQueueSize, pruned_list, scratch, dl,true, _filterIndexingQueueSize);
        } else {
            if(node>=_max_points)
            {
                search_for_point_and_prune(node, _indexingQueueSize, pruned_list, scratch);  
            ...
            // search_for_point_and_prune2(node, _indexingQueueSize, pruned_list, scratch,dl);
        }
        assert(pruned_list.size() > 0);

        // 加锁后修改当前节点的邻居列表，写入图存储
        {
            LockGuard guard(_locks[node]);
            _graph_store->set_neighbours(node, pruned_list);
            assert(_graph_store->get_neighbours(static_cast<location_t>(node)).size() <= _indexingRange);
        }

        // 更新其它节点（互插入操作）
        inter_insert(node, pruned_list, scratch);

        // 输出构建进度（每100000个节点输出一次）
        // if (node_ctr % 100000 == 0) {
...
//进行第一次构建，修改的构建函数
template <typename T, typename TagT, typename LabelT>
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::add_point1(const std::vector<uint32_t>& visit_order,DistList *dl,int64_t node_ctr) {
    // 第一次并行遍历，构建初始图
        auto node = visit_order[node_ctr];
        ...
        if (_filtered_index) {
            if(node>=_max_points)
            {
               ...
               auto cmps = search_for_point_and_prune2(node, _indexingQueueSize, pruned_list, scratch, dl,true, _filterIndexingQueueSize);
               result.first = cmps.first;
               result.second = cmps.second;    
            }
            // search_for_point_and_prune2(node, _indexingQueueSize, pruned_list, scratch, dl,true, _filterIndexingQueueSize);
            ...
            if(node>=_max_points)
            {
               ...
               auto cmps =  search_for_point_and_prune2(node, _indexingQueueSize, pruned_list, scratch,dl);
                result.first = cmps.first;
               result.second = cmps.second;      
            }
            // search_for_point_and_prune2(node, _indexingQueueSize, pruned_list, scratch,dl);
        ...
        // result.first = cmps.first;
        // 更新其它节点（互插入操作）
        result.first+=inter_insert(node, pruned_list, scratch);
...
//进行剪枝
template <typename T, typename TagT, typename LabelT>
unsigned long long Index<T, TagT, LabelT>::globalOptimizationPrune(const std::vector<uint32_t>& visit_order) {
    // 第二次并行遍历，对超过邻居限制的节点进行全局剪枝
    unsigned long long com_prune = 0;
    #pragma omp parallel for schedule(dynamic, 2048) reduction(+: com_prune)
    for (int64_t node_ctr = 0; node_ctr < static_cast<int64_t>(visit_order.size()); node_ctr++) {
        auto node = visit_order[node_ctr];
        if (_graph_store->get_neighbours(static_cast<location_t>(node)).size() > _indexingRange) {
            // 为当前节点准备临时资源
            ScratchStoreManager<InMemQueryScratch<T>> manager(_query_scratch);
            auto scratch = manager.scratch_space();
            tsl::robin_set<uint32_t> dummy_visited(0);
            std::vector<diskann::Neighbor> dummy_pool;
            std::vector<uint32_t> new_out_neighbors;

            // 遍历当前节点的所有邻居，将满足条件的加入临时池
            for (auto cur_nbr : _graph_store->get_neighbours(static_cast<location_t>(node))) {
                if (dummy_visited.find(cur_nbr) == dummy_visited.end() && cur_nbr != node) {
                    float dist = _data_store->get_distance(node, cur_nbr);
                    dummy_pool.emplace_back(Neighbor(cur_nbr, dist));
                    dummy_visited.insert(cur_nbr);
            ...
            // 使用剪枝函数，调整邻居列表
            com_prune+=prune_neighbors(node, dummy_pool, new_out_neighbors, scratch);
            _graph_store->clear_neighbours(static_cast<location_t>(node));
            _graph_store->set_neighbours(static_cast<location_t>(node), new_out_neighbors);
        }
    }
    return com_prune;
}
...
void Index<T, TagT, LabelT>::build_with_data_populated(const std::vector<TagT> &tags)
{
    ...
    diskann::cout << "num_frozen_pts  " << _num_frozen_pts << std::endl;
    if (_nd < 1)
