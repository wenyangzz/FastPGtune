...
void sift_test1B() {
    ...
    cout << "Loading GoundTruth:\n";
    ifstream inputGT(path_gt, ios::binary);
    unsigned int *massQA = new unsigned int[qsize * 1000]; //每个query的1000个近邻的id ，每个近邻用4字节存储
    for (int i = 0; i < qsize; i++) { 
        int t;
        inputGT.read((char *) &t, 4); //前4字节记录了GT的近邻数量
    ...
    for (int i = 0; i < qsize; i++) {
        ...
        inputQ.read((char *) massb, in); //读入一整个向量 也就是128个字节，
...
#pragma omp critical //CRITICAL指令指定一块同一时间只能被一条线程执行的代码区域
            {
                ...
                j1++;
                j2=j1;
                if (j1 % report_every == 0) { //每添加十万条向量，打印一次时间和内存信息
            ...
            appr_alg->addPoint((void *) (mass), (size_t) j2); //j2是label
