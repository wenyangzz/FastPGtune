...
//     }
    ...
    MultiHierarchicalNSW<float> *alg_hnsw7 = new MultiHierarchicalNSW<float>(&space, n , M[1], efConstructions[0]);
    char *data_level02 = alg_hnsw7->data_level0();
    MultiHierarchicalNSW<float> *alg_hnsw8 = new hnswlib::MultiHierarchicalNSW<float>(&space, n , data_level02, M[1], efConstructions[1]);
    MultiHierarchicalNSW<float> *alg_hnsw9 = new hnswlib::MultiHierarchicalNSW<float>(&space, n , data_level02, M[1], efConstructions[2]);
    MultiHierarchicalNSW<float> *alg_hnsw10 = new hnswlib::MultiHierarchicalNSW<float>(&space, n , data_level02, M[1], efConstructions[3]);
    MultiHierarchicalNSW<float> *alg_hnsw11 = new hnswlib::MultiHierarchicalNSW<float>(&space, n , data_level02, M[1], efConstructions[4]);
      MultiHierarchicalNSW<float> *alg_hnsw12 = new hnswlib::MultiHierarchicalNSW<float>(&space, n , data_level02, M[1], efConstructions[5]);
    // HierarchicalNSW<float> *alg_hnsw10 = new hnswlib::HierarchicalNSW<float>(&space, 2 * n);
    // HierarchicalNSW<float> *alg_hnsw11 = new hnswlib::HierarchicalNSW<float>(&space, 2 * n);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        DistList *dl = dist_list_pool->getFreeVisitedList();
         ...
         if(i==4000){
            ...
            // std::cout<<"dl->dists pair size" << sizeof(*(dl->dists_pair_map))<<std::endl;
        }
...
//        // addPoint2(data.data() + d * i, i,-1,dl,cur_id,0);
...
//         DistList *dl = dist_list_pool->getFreeVisitedList();
//         tableint cur_id = alg_hnsw7->get_cur_id(i);
//         dist_compute2 = alg_hnsw7->addPoint2(data.data() + d * i, i,-1,dl,cur_id,0);
//         dist_compute+=dist_compute2;
//         dist_compute2 = alg_hnsw8->addPoint3(data.data() + d * i, i,-1,dl,cur_id,1);
//          dist_compute+= dist_compute2;
//         dist_compute2 = alg_hnsw9->addPoint3(data.data() + d * i, i,-1,dl,cur_id,1);
//         dist_compute+=dist_compute2;
//         dist_compute2 = alg_hnsw10->addPoint3(data.data() + d * i, i,-1,dl,cur_id,1);
//         dist_compute+=dist_compute2;
//         dist_compute2 = alg_hnsw11->addPoint3(data.data() + d * i, i,-1,dl,cur_id,1);
//          dist_compute+= dist_compute2;
//         dist_compute2 = alg_hnsw12->addPoint3(data.data() + d * i, i,-1,dl,cur_id,1);
//         dist_compute+=dist_compute2;
//         dist_list_pool->releaseDistList(dl);
// }
...
//sift_sample_groundtruth.ivecs
    ...
    // MultiHierarchicalNSW<float> *appr_alg = new MultiHierarchicalNSW<float>(&l2space, vecsize, M[0], efConstruction);
    // char *data_level0 = appr_alg->data_level0();
    // MultiHierarchicalNSW<float> *appr_alg2 = new MultiHierarchicalNSW<float>(&l2space, vecsize,data_level0, M[1], efConstruction);
    // MultiHierarchicalNSW<float> *appr_alg3 = new MultiHierarchicalNSW<float>(&l2space, vecsize,data_level0, M[2], efConstruction);
    // MultiHierarchicalNSW<float> *appr_alg4 = new MultiHierarchicalNSW<float>(&l2space, vecsize, data_level0, M[3], efConstruction);
    // MultiHierarchicalNSW<float> *appr_alg5 = new MultiHierarchicalNSW<float>(&l2space, vecsize, data_level0, M[4], efConstruction);
    // MultiHierarchicalNSW<float> *appr_alg6 = new MultiHierarchicalNSW<float>(&l2space, vecsize, data_level0,M[5], efConstruction);
    // MultiHierarchicalNSW<float> *appr_alg7 = new MultiHierarchicalNSW<float>(&l2space, vecsize, data_level0, M[6], efConstruction);
    // MultiHierarchicalNSW<float> *appr_alg8 = new MultiHierarchicalNSW<float>(&l2space, vecsize, data_level0,M[7], efConstruction);
    // MultiHierarchicalNSW<float> *appr_alg9 = new MultiHierarchicalNSW<float>(&l2space, vecsize, data_level0,M[8], efConstruction);
    // MultiHierarchicalNSW<float> *appr_alg10 = new MultiHierarchicalNSW<float>(&l2space, vecsize, data_level0,M[9], efConstruction);
...
#pragma omp critical //CRITICAL指令指定一块同一时间只能被一条线程执行的代码区域
            ...
            // dist_compute+=dist_compute2;
            // dist_compute2 = appr_alg3->addPoint2((void *) (mass), (size_t) j2,-1,dl,cur_id,1);
            // dist_compute+=dist_compute2;
            // dist_compute2 = appr_alg4->addPoint2((void *) (mass), (size_t) j2,-1,dl,cur_id,1);
            // dist_compute+=dist_compute2;
            // dist_compute2 = appr_alg5->addPoint2((void *) (mass), (size_t) j2,-1,dl,cur_id,1);
            // dist_compute+=dist_compute2;
            // dist_compute2 = appr_alg6->addPoint2((void *) (mass), (size_t) j2,-1,dl,cur_id,1);
            // dist_compute+=dist_compute2;
            // dist_compute2 = appr_alg7->addPoint2((void *) (mass), (size_t) j2,-1,dl,cur_id,1);
            // dist_compute+=dist_compute2;
            // dist_compute2 = appr_alg8->addPoint2((void *) (mass), (size_t) j2,-1,dl,cur_id,1);
            // dist_compute+=dist_compute2;
            // dist_compute2 = appr_alg9->addPoint2((void *) (mass), (size_t) j2,-1,dl,cur_id,1);
            // dist_compute+=dist_compute2;
            // dist_compute2 = appr_alg10->addPoint2((void *) (mass), (size_t) j2,-1,dl,cur_id,1);
            // dist_compute+=dist_compute2;
            // dist_list_pool->releaseDistList(dl);
