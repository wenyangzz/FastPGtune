...
namespace hnswlib {
    ...
    class HierarchicalNSW : public AlgorithmInterface<dist_t> { //继承AlgorithmInterface
    public:
...
// #endif
        ...
        linklistsizeint *get_linklist(tableint internal_id, int level) const {  //指定id,获得其第level层的邻居
            return (linklistsizeint *) (linkLists_[internal_id] + (level - 1) * size_links_per_element_);//后面是偏移量，通过其获得第level层的邻居起始地址
        };
