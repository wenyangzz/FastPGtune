...
namespace hnswlib {
    ...
    public:
        ...
        MultiHierarchicalNSW(SpaceInterface<dist_t> *s, size_t max_elements, char *data_level0,size_t M = 16, size_t ef_construction = 200, size_t random_seed = 100) :
                link_list_locks_(max_elements), link_list_update_locks_(max_update_element_locks), element_levels_(max_elements) {
            max_elements_ = max_elements;      
            ...
            //data_level0_memory_ = (char *) malloc(max_elements_ * size_data_per_element_);  //给0层开辟内存 
            data_level0_memory_ = data_level0;//修改四,将data_level0_memory_作为参数传入
        ...
        ~MultiHierarchicalNSW() { 
            if(data_level0_memory_ != nullptr){ //修改4，防止多次删除data_level0_memory_
...
// #endif
        ...
        //     }
        //     int k=0;
        //     int queue_size=top_candidates.size();
        //     std::priority_queue<std::pair<dist_t, tableint>> queue_closest; 
        //     std::vector<std::pair<dist_t, tableint>> return_list; 
        //     while (top_candidates.size() > 0) {
        ...
        unsigned long long addPoint(const void *data_point, labeltype label) {
         ...
         std::pair<unsigned long long,unsigned long long>  addPoint2(const void *data_point, labeltype label,float *dist_array) { //改进后的添加节点
            // print.lock();
            // std::cout<<"dist_array:  "<<&dist_array<<" \ti:  "<<label<<std::endl;
            // print.unlock();
            // unsigned long long dist_coumputes=0;
            std::pair<unsigned long long,unsigned long long> result;
            result = addPoint2(data_point, label,-1,dist_array);
            return result;
        }
        ...
        std::pair<unsigned long long,unsigned long long> addPoint2(const void *data_point, labeltype label, int level,DistList *dl,tableint cur,int flags) { //修改加入float *dist_array
           // std::cout<< omp_get_num_threads  float *dist_array
            std::pair<unsigned long long,unsigned long long> result;
            result.first = 0;
            result.second = 0;
            unsigned long long dist_compute = 0;
            unsigned long long dist_compute_prune = 0;
            tableint cur_c = cur;
            {
                // Checking if the element with the same label already exists
                // if so, updating it *instead* of creating a new element.
            ...
            if ((signed)currObj != -1) {  //非索引中第一个元素
                ...
                for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) { //从当前层到最底层
                    ...
                    currObj = mutuallyConnectNewElement2(data_point, cur_c, top_candidates, level, false,dl,&dist_compute_prune); ////根据top_candidates连接元素
            ...
            result.first = dist_compute;
            result.second = dist_compute_prune;    
            return result;
        };
        unsigned long long getCandidates(const void *data_point,std::vector<std::deque<std::pair<dist_t, tableint>>>&candidates_lists, labeltype label, int level,DistList *dl,tableint cur,int flags) { //修改加入float *dist_array
           // std::cout<< omp_get_num_threads  float *dist_array
            unsigned long long dist_compute = 0;
            tableint cur_c = cur;
            {
            ...
            if ((signed)currObj != -1) {  //非索引中第一个元素
                ...
                for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) { //从当前层到最底层
                    ...
                    std::vector<std::pair<dist_t, tableint>> candidates_list; 
                    
                    while (top_candidates.size() > 0) {
                        candidates_list.push_back(std::pair<dist_t, tableint>(-top_candidates.top().first, top_candidates.top().second));

                        top_candidates.pop();
                    }
                    candidates_lists.insert(candidates_list);
        ...
        void insertPoint(const void *data_point, std::vector<std::deque<std::pair<dist_t, tableint>>>&candidates_lists,labeltype label, int level,DistList *dl,tableint cur,int flags) {
            
        }
        unsigned long long addPoint3(const void *data_point, labeltype label, int level,DistList *dl,tableint cur,int flags) { //修改加入float *dist_array
           // std::cout<< omp_get_num_threads  float *dist_array
            unsigned long long dist_compute = 0;
            tableint cur_c = cur;
            {
